...
//87209302c7f24d56b1d114eadd0ce41f
var thisiBeacon = require('./ibeacon.json');

//var thisUUID = '87209302-C7F2-4D56-B1D1-14EADD0CE41F'; thisMajor = 1; thisMinor =1;
var thisGUID =  thisiBeacon.uuid + thisiBeacon.major + thisiBeacon.minor;
var thisID;
var ibeacon = app.models.ibeacon;

ibeacon.upsert({ guid:thisGUID,uuid:thisiBeacon.uuid, major:thisiBeacon.major, minor:thisiBeacon.minor},
	function( error, _ibeacon) 
	{
		if ( error ) {
			console.log( "error on thisiBeacon upsert " );
		}
		else
		{
			thisID = _ibeacon.id;
...
// http://localhost:3000/api/engagements?filter[where][cBeaconGUID]=87209302c7f24d56b1d114eadd0ce41f15
// http://localhost:3000/api/engagements?filter[where][cBeaconGUID]=87209302c7f24d56b1d114eadd0ce41f15&filter[where][pBeaconGUID]=87209302c7f24d56b1d114eadd0ce41f16
	...
	ibeacon.upsert({ guid:guid, uuid:bleacon.uuid, major:bleacon.major, minor:bleacon.minor},
		function( error, _ibeacon)
		{
			...
			if ( error ) {
				...
				console.log( "upsert success" + thisID + + "" );
				
				var eng = app.models.engagement;
				eng.create({
					cBeaconGUID: thisGUID,
					pBeaconGUID: _ibeacon.guid,
					timestamp: Date.now(),
					rssi: bleacon.rssi,
