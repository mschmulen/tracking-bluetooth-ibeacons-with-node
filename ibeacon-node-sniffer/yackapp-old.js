...
Bleacon.on('discover', function(bleacon) {
	...
	ibeacon.findOrCreate(
		{ where: {uuid:bleacon.uuid, major:bleacon.major, minor:bleacon.minor }  },
		{
		  name: "name",
		  uuid: bleacon.uuid,
		  major: bleacon.major,
		...
		function( error, _ibeacon) 
	{
		...
		if (ibeaconCache[_ibeacon.id] != null ) {
			
			if ( ibeaconCache[_ibeacon.id] != bleacon.proximity) 
			{	
				ibeaconCache[_ibeacon.id] = bleacon.proximiy;
		...
		{
			console.log( " add to cache list " + bleacon.id );
			ibeaconCache[_ibeacon.id] = bleacon.proximity;
		}
		
		
	});
	
	//thats cool but lets do some engeements and track people who come into the office
